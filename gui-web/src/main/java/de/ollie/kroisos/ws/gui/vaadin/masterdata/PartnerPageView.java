...
/**
 * A view for paginated partner lists.
 *
 * GENERATED CODE !!! DO NOT CHANGE !!!
 */
@Generated
@Route(PartnerPageView.URL)
@RequiredArgsConstructor
public class PartnerPageView extends VerticalLayout implements BeforeEnterObserver, HasUrlParameter<String> {

	public static final String URL = "kroisos-ws/masterdata/partners";

	private static final Logger logger = LogManager.getLogger(PartnerPageView.class);

	@Autowired(required = false)
	private MasterDataGridFieldRenderer<Partner> masterDataGridFieldRenderer;
	...
	public void beforeEnter(BeforeEnterEvent beforeEnterEvent) {
		...
		grid
				...
				.setHeader(resourceManager.getLocalizedString("PartnerPageView.grid.header.bezeichnung.label", session.getLocalization()))
				.setSortable(true);
		...
		add(
				new HeaderLayout(
						...
						resourceManager.getLocalizedString("PartnerPageView.header.label", session.getLocalization()),
	...
	private Object getHeaderString(String fieldName, Partner aTable, Supplier<?> f) {
	...
	private void enabledButtons(SelectionEvent<Grid<Partner>, Partner> event) {
	...
	protected void onAttach(AttachEvent attachEvent) {
		logger.info("Partner page layout opened for user '{}'.", session.getUserName());
	...
	private void addRecord() {
		getUI().ifPresent(ui -> ui.navigate(PartnerMaintenanceView.URL));
	}
	...
	private void duplicateRecord() {
		grid.getSelectedItems().stream().findFirst().ifPresent(model -> {
			...
			getUI().ifPresent(ui -> ui.navigate(PartnerMaintenanceView.URL, parameters));
		});
	}
