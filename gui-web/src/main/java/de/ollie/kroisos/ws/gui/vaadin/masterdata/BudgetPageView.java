...
/**
 * A view for paginated budget lists.
 *
 * GENERATED CODE !!! DO NOT CHANGE !!!
 */
@Generated
@Route(BudgetPageView.URL)
@RequiredArgsConstructor
public class BudgetPageView extends VerticalLayout implements BeforeEnterObserver, HasUrlParameter<String> {

	public static final String URL = "kroisos-ws/masterdata/budgets";

	private static final Logger logger = LogManager.getLogger(BudgetPageView.class);

	@Autowired(required = false)
	private MasterDataGridFieldRenderer<Budget> masterDataGridFieldRenderer;
	...
	public void beforeEnter(BeforeEnterEvent beforeEnterEvent) {
		...
		grid = new Grid<>();
		grid
				.addColumn(model -> getHeaderString("KUERZEL", model, () -> model.getKuerzel()))
				.setHeader(resourceManager.getLocalizedString("BudgetPageView.grid.header.kuerzel.label", session.getLocalization()))
				.setSortable(true);
		...
		add(
				new HeaderLayout(
						...
						resourceManager.getLocalizedString("BudgetPageView.header.label", session.getLocalization()),
	...
	private Object getHeaderString(String fieldName, Budget aTable, Supplier<?> f) {
	...
	private void enabledButtons(SelectionEvent<Grid<Budget>, Budget> event) {
	...
	protected void onAttach(AttachEvent attachEvent) {
		logger.info("Budget page layout opened for user '{}'.", session.getUserName());
	...
	private void addRecord() {
		getUI().ifPresent(ui -> ui.navigate(BudgetMaintenanceView.URL));
	}
	...
	private void duplicateRecord() {
		grid.getSelectedItems().stream().findFirst().ifPresent(model -> {
			...
			getUI().ifPresent(ui -> ui.navigate(BudgetMaintenanceView.URL, parameters));
		});
	}
