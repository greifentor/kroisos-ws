...
/**
 * A view for paginated buchungbudget lists.
 *
 * GENERATED CODE !!! DO NOT CHANGE !!!
 */
@Generated
@Route(BuchungBudgetPageView.URL)
@RequiredArgsConstructor
public class BuchungBudgetPageView extends VerticalLayout implements BeforeEnterObserver, HasUrlParameter<String> {

	public static final String URL = "kroisos-ws/masterdata/buchung_budgets";

	private static final Logger logger = LogManager.getLogger(BuchungBudgetPageView.class);

	@Autowired(required = false)
	private MasterDataGridFieldRenderer<BuchungBudget> masterDataGridFieldRenderer;
	...
	private Grid<BuchungBudget> grid;
	...
	public void beforeEnter(BeforeEnterEvent beforeEnterEvent) {
		...
		grid = new Grid<>();
		grid
				.addColumn(model -> getHeaderString("BUCHUNG", model, () -> model.getBuchung()))
				.setHeader(resourceManager.getLocalizedString("BuchungBudgetPageView.grid.header.buchung.label", session.getLocalization()))
				.setSortable(true);
		grid
				.addColumn(model -> getHeaderString("BUDGET", model, () -> model.getBudget()))
				.setHeader(resourceManager.getLocalizedString("BuchungBudgetPageView.grid.header.budget.label", session.getLocalization()))
				.setSortable(true);
		grid
				.addColumn(model -> getHeaderString("BETRAG", model, () -> model.getBetrag()))
				.setHeader(resourceManager.getLocalizedString("BuchungBudgetPageView.grid.header.betrag.label", session.getLocalization()))
				.setSortable(true);
		...
		add(
				new HeaderLayout(
						...
						resourceManager.getLocalizedString("BuchungBudgetPageView.header.label", session.getLocalization()),
	...
	private Object getHeaderString(String fieldName, BuchungBudget aTable, Supplier<?> f) {
	...
	private void enabledButtons(SelectionEvent<Grid<BuchungBudget>, BuchungBudget> event) {
	...
	protected void onAttach(AttachEvent attachEvent) {
		logger.info("BuchungBudget page layout opened for user '{}'.", session.getUserName());
	...
	private void addRecord() {
		getUI().ifPresent(ui -> ui.navigate(BuchungBudgetMaintenanceView.URL));
	}
	...
	private void duplicateRecord() {
		grid.getSelectedItems().stream().findFirst().ifPresent(model -> {
			...
			getUI().ifPresent(ui -> ui.navigate(BuchungBudgetMaintenanceView.URL, parameters));
		});
	}
