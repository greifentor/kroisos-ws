...
/**
 * A dialog to edit Partner details.
 *
 * GENERATED CODE !!! DO NOT CHANGE !!!
 */
@Generated
@Route(PartnerMaintenanceView.URL)
...
public class PartnerMaintenanceView extends AbstractMasterDataBaseLayout implements PartnerDetailsLayout.Observer {

	public static final String URL = "kroisos-ws/masterdata/partners/details";

	private static final Logger logger = LogManager.getLogger(PartnerMaintenanceView.class);

	@Autowired(required = false)
	private MaintenanceViewRenderer<Partner> maintenanceViewRenderer;
	@Autowired(required = false)
	private DetailsLayoutComboBoxItemLabelGenerator<Partner> comboBoxItemLabelGenerator;
	...
	private Partner createNewModel() {
		return new Partner();
	}
		...
		add(
				new HeaderLayout(
						...
						buttonFactory.createLogoutButton(resourceManager, this::getUI, session, logger),
								resourceManager.getLocalizedString("PartnerMaintenanceView.header.prefix.label", session.getLocalization()) + getHeaderSuffix(model),
								HeaderLayoutMode.PLAIN),
				getDetailsLayout(model));
	}

	private String getHeaderSuffix(Partner model) {
		return maintenanceViewRenderer != null
				? maintenanceViewRenderer.getHeaderSuffix(model)
				: "" + model.getBezeichnung();
	}

	private AbstractMasterDataBaseLayout getDetailsLayout(Partner model) {
		return new PartnerDetailsLayout(buttonFactory, model, service, resourceManager, session, this, comboBoxItemLabelGenerator);
	}
