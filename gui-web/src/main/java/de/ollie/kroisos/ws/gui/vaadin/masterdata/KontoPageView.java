...
/**
 * A view for paginated konto lists.
 *
 * GENERATED CODE !!! DO NOT CHANGE !!!
 */
@Generated
@Route(KontoPageView.URL)
@RequiredArgsConstructor
public class KontoPageView extends VerticalLayout implements BeforeEnterObserver, HasUrlParameter<String> {

	public static final String URL = "kroisos-ws/masterdata/kontos";

	private static final Logger logger = LogManager.getLogger(KontoPageView.class);

	@Autowired(required = false)
	private MasterDataGridFieldRenderer<Konto> masterDataGridFieldRenderer;
	...
	public void beforeEnter(BeforeEnterEvent beforeEnterEvent) {
		...
		grid
				...
				.setHeader(resourceManager.getLocalizedString("KontoPageView.grid.header.kuerzel.label", session.getLocalization()))
				.setSortable(true);
		...
		add(
				new HeaderLayout(
						...
						resourceManager.getLocalizedString("KontoPageView.header.label", session.getLocalization()),
	...
	private Object getHeaderString(String fieldName, Konto aTable, Supplier<?> f) {
	...
	private void enabledButtons(SelectionEvent<Grid<Konto>, Konto> event) {
	...
	protected void onAttach(AttachEvent attachEvent) {
		logger.info("Konto page layout opened for user '{}'.", session.getUserName());
	...
	private void addRecord() {
		getUI().ifPresent(ui -> ui.navigate(KontoMaintenanceView.URL));
	}
	...
	private void duplicateRecord() {
		grid.getSelectedItems().stream().findFirst().ifPresent(model -> {
			...
			getUI().ifPresent(ui -> ui.navigate(KontoMaintenanceView.URL, parameters));
		});
	}
